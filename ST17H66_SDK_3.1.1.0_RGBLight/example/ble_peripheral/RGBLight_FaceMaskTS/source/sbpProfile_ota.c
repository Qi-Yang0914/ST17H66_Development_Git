...
#else
bStatus_t	simpleProfile_Notify(attHandleValueNoti_t* pNoti)
{
	uint16	connHandle;
	uint16	notfEnable;

	GAPRole_GetParameter(GAPROLE_CONNHANDLE, &connHandle);
	notfEnable	=	GATTServApp_ReadCharCfg(connHandle, simpleProfileChar2Config);
	...
	if(notfEnable == GATT_CLIENT_CFG_NOTIFY)
	{
		pNoti->handle	=	simpleProfileAttrTbl[SIMPLEPROFILE_CHAR2_HANDLE].handle;
