...
void __attribute__((used)) hal_ADC_IRQHandler(void)
{
	...
	static	uint16	n_cnt	=	0;
	status = GET_IRQ_STATUS;
#if 1
	if(LC_Mic_Sample_Busy == 1){
		volatile	uint32	voice_int_status	=	GET_IRQ_STATUS;
		MASK_VOICE_INT;
		if(voice_int_status & BIT(8)){
			int	n;
			for(n = 0;n < HALF_VOICE_WORD_SIZE;n++){
				voice_data[n]	=	(uint32)(read_reg(VOICE_BASE + n*4));
			}
			CLEAR_VOICE_HALF_INT;
			while(IS_CLEAR_VOICE_HALF_INT){}
		...
		if(voice_int_status & BIT(9)){
			int n;
			for(n = 0;n < HALF_VOICE_WORD_SIZE;n++){
				voice_data[n]	=	(uint32)(read_reg(VOICE_MID_BASE + n*4));
			}
			CLEAR_VOICE_FULL_INT;
			while(IS_CLEAR_VOICE_FULL_INT)	{}
		...
		{				
			...
			if(mAdc_Ctx.continue_mode == FALSE || n_cnt > 10){
				n_cnt	=	0;
				hal_adc_stop();
			}
		}	
...
#else
    *(volatile unsigned int *)0x40050034 &= 0xfffffc00;
    if (status != 0)
    {
        if (((status & 0xFC) == mAdc_Ctx.all_channel) && (mAdc_Ctx.all_channel > 0))
        {
            for (ch = 2; ch <= ADC_CH9; ch++)
            {
                if (status & BIT(ch))
    ...
    *(volatile unsigned int *)0x40050034 |= 0x000003fc;
	#endif
}
