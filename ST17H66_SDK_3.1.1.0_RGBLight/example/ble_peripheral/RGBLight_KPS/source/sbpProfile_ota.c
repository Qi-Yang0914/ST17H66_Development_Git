...
static uint8 simpleProfileChar1[LC_RXD_VALUE_LEN]	=	{0,};			// Characteristic 1 Value
...
static uint8 simpleProfileChar2[LC_RXD_VALUE_LEN]	= 	{0,};			// Characteristic 2 Value
...
bStatus_t SimpleProfile_GetParameter( uint8 param, void *value )
{
	...
	{
		case SIMPLEPROFILE_CHAR1:
			VOID osal_memcpy( value, simpleProfileChar1, LC_RXD_VALUE_LEN);    
		break;

//		case SIMPLEPROFILE_CHAR2:
//			VOID osal_memcpy( value, simpleProfileChar2, LC_RXD_VALUE_LEN );    
//		break;      
		...
		switch ( uuid )
		{
			case SIMPLEPROFILE_CHAR1_UUID:
				...
				if ( status == SUCCESS ){
					...
					VOID osal_memcpy(LC_App_Set_Param.app_write_data, pValue, len);
					LC_App_Set_Param.app_write_len	=	len;
					LOG("receive data = 0x ");
					LOG_DUMP_BYTE(pCurValue, len);
					osal_set_event(LC_Ui_Led_Buzzer_TaskID, UI_EVENT_LEVEL3);
					notifyApp = SIMPLEPROFILE_CHAR1;   
				}		 	  	
			break;
			
			case GATT_CLIENT_CHAR_CFG_UUID:
				LOG("Enable/Disable Notity 0=0\n");
	...
	{  
		case SIMPLEPROFILE_CHAR2:
			...
			if ( notfEnable & GATT_CLIENT_CFG_NOTIFY ){
				...
				LC_App_Set_Param.app_notify_len	=	len;
